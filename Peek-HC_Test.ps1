$hvServerData = $null
$viServerData = $null
$hcPathHome = "C:\Scripts\Peek-HC\Test"
$hcPathOneDrive = "D:\TVG_VDI_HC\OneDrive - kyndryl\TVG VDI HC Dev"
...
$hvServerData = Connect-HVServer -Server port142vms -Credential $vmaCred -ErrorVariable "hvError"
if ($null -ne $hvServerData) {
    $hvData = getHorizonHc -HvObject $hvServerData
    $hvBody = createHtmlBody -Object $hvData
}
else {
    $hvBody = createErrorTable -Type Broker -Name "PORT142VMS"
...
$viServerData = Connect-VIServer -Server port140vma -Credential $vmaCred -ErrorVariable "viError"
if ($null -ne $viServerData) {
    ...
    $viBody = createErrorTable -Type vCenter -Name "PORT140VMA"
